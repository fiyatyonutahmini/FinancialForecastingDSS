...
namespace FinancialForecastingDSS.indicators
{
    class Stochastics
    {
        ...
        public Stochastics(string code, DateTime targetDate, int fastKPeriod = 14, int fastDPeriod = 3, int slowDPeriod = 3, int numberOfData = 1, bool mapReduce = false)
        {
            if (fastKPeriod <= 0)
            {
                throw new IndicatorException("Fast K period must be positive.");
            }
            else if (fastDPeriod <= 0)
            {
                throw new IndicatorException("Fast D period must be positive.");
            }
            else if (slowDPeriod <= 0)
            {
                throw new IndicatorException("Slow D period must be positive.");
            }
            ...
            {
                throw new IndicatorException("Number of datas to be processed cannot be less than Fast K period.");
            }
